...
1. **開始位置**: `opts.startX/Y` が渡された場合はそれを使用。未指定の場合はランダム（`startX: 100~900, startY: 100~500`）。実際の運用では各 injectCode が `page.evaluate(() => window.__lastMouseX/Y)` で現在のマウス位置を読み取り、`opts` として渡す
...
#### humanClick(page, element, opts)

1. `element.boundingBox()` で要素の矩形を取得
2. 矩形の中心からランダムオフセット（要素内の 30%〜70% 範囲）を目標点に設定
3. `humanMove(page, targetX, targetY, opts)` で目標点へ移動（`opts` には現在のマウス位置が渡される）
4. 50〜150ms のホバーポーズ
5. `page.mouse.down()` → 40〜100ms 待機 → `page.mouse.up()`（人間のクリック間隔）

#### humanHover(page, element, opts)

1. `element.boundingBox()` で矩形を取得（取得不可なら何もしない）
2. ランダムオフセット目標点へ `humanMove(page, targetX, targetY, opts)` で移動（クリックなし）
...
- `dist/mouse-helper.js` は DOM `mousemove` イベントを捕捉し、現在のマウス座標を `window.__lastMouseX/Y` に保存する
- 各操作（click/hover/type）の injectCode 冒頭で `page.evaluate(() => ({ startX: window.__lastMouseX, startY: window.__lastMouseY }))` を実行し、現在のカーソル位置を読み取る
- 読み取った位置は `humanMove` の `opts.startX/Y` として渡され、Bezier 曲線の開始点として使用される
- `page.mouse.move()` によるプログラム的なマウス移動でも DOM `mousemove` イベントが発火し、`window.__lastMouseX/Y` が自動更新される
- これにより、連続した `hover` → `click` → `type` などの操作が、毎回直前のカーソル位置から目標へ向かって移動する

### 5.5 open 時のマウスヘルパー自動注入
