...
// Define BoundingBox interface since Playwright doesn't export it
...
const getElementBox = async (
  ...
  try {
    ...
    const objectId = await element.evaluate((el) => {
      // Get the remote object ID for CDP
      return (el as any)[Symbol.for('pwPlaywrightRemoteObjectId')] || undefined
    })
    ...
    if (!relativeToMainFrame) {
      // For iframe handling, we'll use the simpler approach with Playwright
      // Playwright handles frame coordinates differently
      const frameBox = await element.evaluate((el) => {
...
const generateTimestamps = (
  ...
  const timedVectors: TimedVector[] = vectors.map((vector) => ({
    ...vector,
    timestamp: 0
  }))
  ...
  for (let i = 0; i < timedVectors.length; i++) {
    ...
    const P2 =
			i === timedVectors.length - 1 ? timedVectors[i] : timedVectors[i + 1]
    const P3 =
			i === timedVectors.length - 1 ? timedVectors[i] : timedVectors[i + 1]
...
export const createCursor = (
  ...
  const tracePath = async (
    ...
    for (const v of vectors) {
      try {
        ...
        // Use Playwright's native mouse movement
  ...
  const randomMove = async (options?: RandomMoveOptions): Promise<void> => {
    ...
    try {
      ...
      await delay(
        optionsResolved.moveDelay *
					(optionsResolved.randomizeMoveDelay ? Math.random() : 1)
      )
  ...
  const actions: GhostCursor = {
    ...
    async click (
      ...
      try {
        ...
        // Use Playwright's native click method
    ...
    async move (
      ...
      const go = async (iteration: number): Promise<void> => {
        ...
        const destination =
					optionsResolved.destination !== undefined
    ...
    async scrollIntoView (
      ...
      const targetBox = {
        ...
        bottom:
					Math.min(marginedBoxRelativeToDoc.bottom, docHeight) -
					scrollPositionTop,
        right:
					Math.min(marginedBoxRelativeToDoc.right, docWidth) -
					scrollPositionLeft
      }
      ...
      try {
        if (scrollSpeed === 100 && optionsResolved.inViewportMargin <= 0) {
          try {
            // Use Playwright's native scrollIntoViewIfNeeded
    ...
    async scroll (delta: Partial<Vector>, options?: ScrollOptions) {
      ...
      const [largerDistance, shorterDistance] =
				largerDistanceDir === 'x' ? [deltaX, deltaY] : [deltaY, deltaX]
      ...
      const largerDistanceScrollStep =
				scrollSpeed < EXP_SCALE_START
				  ? scrollSpeed
				  : scale(
				    scrollSpeed,
				    [EXP_SCALE_START, 100],
				    [EXP_SCALE_START, largerDistance]
				  )
      ...
      for (let i = 0; i < numSteps; i++) {
        ...
        // Use Playwright's native wheel method
    ...
    async scrollTo (destination: ScrollToDestination, options?: ScrollOptions) {
      ...
      const { docHeight, docWidth, scrollPositionTop, scrollPositionLeft } =
				await page.evaluate(() => ({
				  docHeight: document.body.scrollHeight,
				  docWidth: document.body.scrollWidth,
				  scrollPositionTop: window.scrollY,
    ...
    async getElement (
      ...
      if (typeof selector === 'string') {
        ...
        if (selector.startsWith('//') || selector.startsWith('(//')) {
          ...
          elem = (await page.$(xpathSelector)) as ElementHandle<Element> | null
        } else {
          if (optionsResolved.waitForSelector !== undefined) {
          ...
          elem = (await page.$(selector)) as ElementHandle<Element> | null
        }
        if (elem === null) {
          throw new Error(
        ...
        // ElementHandle
        return selector as ElementHandle<Element>
      }
    }
  }
